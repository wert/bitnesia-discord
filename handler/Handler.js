...
class Handler {
  ...
  register() {
    ...
    this.client.on('message', async message => {
      ...
      var t = cmd.permission.includes("everyone") ? true : message.member.roles.cache.some(role => cmd.permission.includes(role.name));
      if(!t) {
          const msg = Utils.noPermissions("Error",cmd.permission.join(", "));
