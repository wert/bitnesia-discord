...
module.exports = class extends Command {
  ...
  async run(message, args) {
      if(args.length == 0) {
          const msg = Utils.ErrorMsg("Incorrect Usage","You must use the following format: `kick <@user> [reason]` \n[] is optional, <> is required");
      ...
      .setTimestamp()
      .setAuthor("Kick Log", 'https://i.imgur.com/8WQ6OdT.png', 'https://bitnesia.net')
      .addField("Executor",message.author.tag + " `(" + message.author.id + ")`")
      .addField("Target", target.user.tag + " `(" + target.user.id + ")`")
      .addField("Reason", reason)
      .setColor(colours.red_light);

      target.kick("Executor: " + message.author.tag + " ID: (" + message.author.id + ") Reason: "+reason);
